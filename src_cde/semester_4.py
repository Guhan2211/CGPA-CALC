...
class semester_4:
    ...
    def sem4(self):
        ...
        for i in range(4,posr+1):
            ...
            gpa=float(num/deno)
            #print('%.2f'%gpa)
            gpa_index4.append(float('%.2f'%gpa))
            #print(gpa_index4)
        ...
        for i in range(4,posr+1):
            sh.cell(row=i,column=pos+2).value=gpa_index4[i-4]
        ...
        if sh.cell(row=4,column=pos+2).value!=None:
            msg.showinfo("SUCCESS","GPA VALUES UPDATED FOR SEMESTER_4 IN 'sem4' SHEET!!")
        ...
        g1=pd.read_excel('sem4.xlsx',sheet_name="Sheet1",skiprows=2)
        ...
        #print(rlis)
        g=rlis.plot(kind='bar',title="4th SEM analysis",x='Subject',y='percentage')
