...
class cgpa7:
    ...
    def cg7(self):
        ...
        for i in range(1,no_of_grades+1):
            sum_up=((float(sem1_index[i-1])+float(sem2_index[i-1])+float(sem3_index[i-1])+float(sem4_index[i-1])+float(sem5_index[i-1])+float(sem6_index[i-1])+float(sem7_index[i-1]))/float(7))
        ...
        for i in range(4,cnt+1):
            ...
            sh.cell(row=i,column=10).value=cgpa_index[i-4]
        ...
        sh.cell(row=3,column=10).value='CGPA'
