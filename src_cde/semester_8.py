...
class semester_8:
    ...
    def sem8(self):
        ...
        for i in range(4,posr+1):
            ...
            gpa=float(num/deno)
            #print('%.2f'%gpa)
            gpa_index8.append(float('%.2f'%gpa))
            #print(gpa_index8)
        ...
        for i in range(4,posr+1):
            sh.cell(row=i,column=pos+2).value=gpa_index8[i-4]
        ...
        if sh.cell(row=4,column=pos+2).value!=None:
            msg.showinfo("SUCCESS","GPA VALUES UPDATED FOR SEMESTER_8 IN 'sem8' SHEET!!")
        ...
        g1=pd.read_excel('sem8.xlsx',sheet_name="Sheet1",skiprows=2)
        ...
        #print(rlis)
        g=rlis.plot(kind='bar',title="8th SEM analysis",x='Subject',y='percentage')
