...
class semester_5:
    ...
    def sem5(self):
        ...
        for i in range(4,posr+1):
            ...
            gpa=float(num/deno)
            #print('%.2f'%gpa)
            gpa_index5.append(float('%.2f'%gpa))
            #print(gpa_index5)
        ...
        for i in range(4,posr+1):
            sh.cell(row=i,column=pos+2).value=gpa_index5[i-4]
        ...
        if sh.cell(row=4,column=pos+2).value!=None:
            msg.showinfo("SUCCESS","GPA VALUES UPDATED FOR SEMESTER_5 IN 'sem5' SHEET!!")
        ...
        g1=pd.read_excel('sem5.xlsx',sheet_name="Sheet1",skiprows=2)
        ...
        #print(rlis)
        g=rlis.plot(kind='bar',title="5th SEM analysis",x='Subject',y='percentage')
