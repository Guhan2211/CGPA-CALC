...
class semester_6:
    ...
    def sem6(self):
        ...
        for i in range(4,posr+1):
            ...
            gpa=float(num/deno)
            #print('%.2f'%gpa)
            gpa_index6.append(float('%.2f'%gpa))
            #print(gpa_index6)
        ...
        for i in range(4,posr+1):
            sh.cell(row=i,column=pos+2).value=gpa_index6[i-4]
        ...
        if sh.cell(row=4,column=pos+2).value!=None:
            msg.showinfo("SUCCESS","GPA VALUES UPDATED FOR SEMESTER_6 IN 'sem6' SHEET!!")
        ...
        g1=pd.read_excel('sem6.xlsx',sheet_name="Sheet1",skiprows=2)
        ...
        #print(rlis)
        g=rlis.plot(kind='bar',title="6th SEM analysis",x='Subject',y='percentage')
