...
class semester_1:
    ...
    def sem1(self):
        ...
        for i in range(4,posr+1):
            ...
            gpa=float(num/deno)
            #print('%.2f'%gpa)
            gpa_index1.append(float('%.2f'%gpa))
            #print(gpa_index1)
        ...
        for i in range(3,pos+1):
            ...
            for r in range(4,posr+1):
                ...
                if sh.cell(row=r,column=i).value not in gr:#and != 'WH'
                    subarr=subarr+1
        ...
        for i in range(4,posr+1):
            sh.cell(row=i,column=pos+2).value=gpa_index1[i-4]
        ...
        if sh.cell(row=4,column=pos+2).value!=None:
            msg.showinfo("SUCCESS","GPA VALUES UPDATED FOR SEMESTER_1 IN 'sem1' SHEET!!")
        ...
        g1=pd.read_excel('sem1.xlsx',sheet_name="Sheet1",skiprows=2)
        ...
        #print(rlis)
        g=rlis.plot(kind='bar',title="1st SEM analysis",x='Subject',y='percentage')
