...
class cgpa8:
    ...
    def cg8(self):
        ...
        wb7=op.load_workbook('sem7.xlsx')
        wb8=op.load_workbook('sem8.xlsx')
        sh1=wb1['Sheet1']
        sh2=wb2['Sheet1']
        sh3=wb3['Sheet1']
        sh4=wb4['Sheet1']
        ...
        for i in range(4,cnt+1):
            ...
            sem7_index.append(sh7.cell(row=i,column=sh7.max_column).value)
            sem8_index.append(sh8.cell(row=i,column=sh8.max_column).value)

        no_of_grades=len(sem1_index)

        for i in range(1,no_of_grades+1):
            sum_up=float((float(sem1_index[i-1])+float(sem2_index[i-1])+float(sem3_index[i-1])+float(sem4_index[i-1])+float(sem5_index[i-1])+float(sem6_index[i-1])+float(sem7_index[i-1])+float(sem8_index[i-1]))/float(8))
        ...
        for i in range(4,cnt+1):
            ...
            sh.cell(row=i,column=9).value=sem7_index[i-4]
            sh.cell(row=i,column=10).value=sem8_index[i-4]

            sh.cell(row=i,column=11).value=cgpa_index[i-4]
        ...
        sh.cell(row=3,column=9).value='SEMESTER_7'
        sh.cell(row=3,column=10).value='SEMESTER_8'

        sh.cell(row=3,column=11).value='CGPA'
