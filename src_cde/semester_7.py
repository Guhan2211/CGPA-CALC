...
class semester_7:
    ...
    def sem7(self):
        ...
        for i in range(4,posr+1):
            ...
            gpa=float(num/deno)
            #print('%.2f'%gpa)
            gpa_index7.append(float('%.2f'%gpa))
            #print(gpa_index7)
        ...
        for i in range(4,posr+1):
            sh.cell(row=i,column=pos+2).value=gpa_index7[i-4]
        ...
        if sh.cell(row=4,column=pos+2).value!=None:
            msg.showinfo("SUCCESS","GPA VALUES UPDATED FOR SEMESTER_7 IN 'sem7' SHEET!!")
        ...
        g1=pd.read_excel('sem7.xlsx',sheet_name="Sheet1",skiprows=2)
        ...
        #print(rlis)
        g=rlis.plot(kind='bar',title="7th SEM analysis",x='Subject',y='percentage')
